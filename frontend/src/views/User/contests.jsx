      ...
      {contests.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {contests.map((contest) => (
            <Link to={`/user/contest/${contest._id}`} key={contest._id} className="bg-white shadow-md rounded-lg p-6">
              ...
              {new Date() >= new Date(contest.startTime) && new Date() <= new Date(contest.endTime)?<p className='text-sm mt-3 text-green-500'>Ongoing</p>:<></>}
            </Link>
          ))}
        </div>
      ) : (
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="mt-2 text-gray-500">Please solve practice questions</p>
        </div>
      )}
    </div>
    </div>
